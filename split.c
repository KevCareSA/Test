...
void place_nds(partition **start_h, cmd_t **last_h, char *ln)
{
	...
	line = _strtok(ln, delim);
	do {
		line = rep_sep(line, 1);
		cmdln_add_end(last_h, line);
		line = _strtok(NULL, delim);
	} while (line != NULL);
}
...
/**
 * split_to_arr - tokenizes the ln string
 *
 * @ln: ln string.
...
char **split_to_arr(char *ln)
{
	...
	while (token != NULL)
	{
		...
		{
			buf += mybuff + 8;
			tokens = dprllc(tokens, idx, sizeof(char *) * buf);
			if (tokens == NULL)
			{
				display_err("malloc(): failed to allocate memory\n");
				exit(EXIT_FAILURE);
